...
**支持的发布站点**（platform: atomic，MCP 可用）：

| 站点               | blog_id     | 定位               | 英文分类 ID                     | 中文分类 ID          |
| ------------------ | ----------- | ------------------ | ------------------------------- | -------------------- |
| traffictalking.com | `247107897` | DeepClick 营销博客 | `132592`（Conversion Insights） | `132594`（转化洞察） |
| googlepwa.blog     | `235795097` | PWA 技术站         | 需确认                          | 需确认               |
| androidpwa.com     | `236140245` | 安卓 PWA 出海站    | 需确认                          | 需确认               |

> ❌ **免费站点（MCP 不可用）**：androidpwa.wordpress.com、webappcn.wordpress.com
> 如需发布到免费站点，需用户提供 Application Password 通过 REST API 发布。

用户未指定站点时，默认发布到 **traffictalking.com**（`247107897`）。
批量模式（定时任务）下，按用户在启动参数中指定的站点列表逐一发布。

**⚠️ wpcom-mcp 已知限制（截至 2026-03）**：

| 参数                            | 问题                                                   | 解决方案                            |
| ------------------------------- | ------------------------------------------------------ | ----------------------------------- |
| `categories`                    | 传数组 `[132592]` 报错；改传**单个整数** `132592`      | `"categories": 132592`（无方括号）  |
| `meta`（SEO 字段）              | 传对象时报错，无法通过 MCP 设置                        | 发布后在 WP 后台 Rank Math 手动设置 |
| `title` / `content` / `excerpt` | **必须直接传字符串**，不要用 `{"raw": "..."}` 对象格式 | 正确示例：`"title": "My Title"`     |

**发布状态策略**：

- **交互模式**：先发 `draft`，输出预览，等用户确认后再更新为 `publish`
- **批量/cron 模式**（含 `--batch` 或无交互上下文）：直接发 `publish`，跳过确认步骤
...
**步骤 5.3 - 仅交互模式**：显示 draft 链接供用户确认，收到确认后执行：
